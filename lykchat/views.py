...
class Base():
    ...
    def _get_friend_info(self):
        ...
        if (self.friend_dict != {} and self.friend_dict) or self.friend_list != {'UserName': [], 'NickName': [],
                                                                                 'Alias': [], 'Sex': [],
                                                                                 'RemarkName': []}:
            if change_time >= 3600:
    ...
    def _send_msg(self, tousername, content, filename='', msgType='txt', call_type='', post_field='UserName'):
        ...
        send_result_dict = send_msg.send(content, msgType=msgType, filename=filename, tousername=tousername,
...
class Interface(Base):
    def sendmsg(self, request):
        ...
        send_result = self._send_msg(tousername=tousername, content=content, msgType=request_dict['type'],
                                     filename=filename, call_type='interface', post_field=friendfield)
        return render(request, resultpage, {'result': send_result})
    ...
    def check_login(self, request):
        ...
        if send_result == {}:
            ...
            if status == 222:
                ...
                if login_min >= 60 * 24:
                    login_min = str(login_min // 60 // 24) + '天' + str(login_min // 60 % 24) + '小时' + str(
                        login_min % 60) + '分钟'
                elif login_min >= 60:
